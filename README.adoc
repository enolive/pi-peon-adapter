...
| —
| 🔇
| Sourced from the permission system's decision channel; `tool_name` carries the decision's `surface`. Only `result=allow` decisions are forwarded; deny decisions are skipped. PeonPing plays no sound, but will remove "needs aproval" in compatible terminals.
|===
